...
<div class="container mt-5 form">
    ...
    <form:form method="post" action="${pageContext.request.contextPath}/view/signup/service" modelAttribute="newServiceUsage">
        ...
        <label>Mã máy</label>
        <form:select class="form-control" path="services.id">
            <form:option value=""> Chọn dịch vụ </form:option>
            <c:forEach var="service" items="${service}">
                <form:option value="${service.id}">${service.id}</form:option>
            </c:forEach>
        </form:select>
        <div class="form-group">
            <label>Ngày sử dụng</label>
            <form:input type="date" class="form-control" path="usageDate"/>
            <form:errors path="usageDate" cssClass="text-danger"/>
        </div>
        <div class="form-group">
            <label>Giờ sử dụng</label>
            <form:input type = "time" class="form-control" path="usageTime"/>
            <form:errors path="usageTime" cssClass="text-danger"/>
        </div>
        <div class="form-group">
